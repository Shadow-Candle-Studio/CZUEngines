...
rem Register the engine root as a system environment variable
set UE_ENGINE_ROOT=%~dp0
rem Remove trailing backslash if present
if %UE_ENGINE_ROOT:~-1%==\ set UE_ENGINE_ROOT=%UE_ENGINE_ROOT:~0,-1%
echo Setting UE_ENGINE_ROOT to %UE_ENGINE_ROOT%
reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v UE_ENGINE_ROOT /t REG_SZ /d "%UE_ENGINE_ROOT%" /f
if %ERRORLEVEL% NEQ 0 goto error
